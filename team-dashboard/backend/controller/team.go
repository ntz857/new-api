...
func ModelStats(c *gin.Context) {
	...
	start, err := time.Parse(layout, startStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "start 日期格式错误"})
		return
	}
	end, err := time.Parse(layout, endStr)
	if err != nil {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "end 日期格式错误"})
		return
	}
	if end.Before(start) {
		c.JSON(http.StatusBadRequest, gin.H{"success": false, "message": "end 不能早于 start"})
	...
	stats, err := model.GetDailyModelStats(memberIDs, start, end)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "统计查询失败"})
		return
	}
...
func GroupStats(c *gin.Context) {
	...
	groupStats, err := model.GetDailyGroupStats(memberIDs, start, end)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"success": false, "message": "分组统计查询失败"})
		return
	}
