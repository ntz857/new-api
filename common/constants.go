...
// Only "default" and "apini" are accepted; other values are silently ignored.
func SetTheme(t string) {
	if t == "default" || t == "apini" {
		themeValue.Store(t)
	}
}

// ThemeAwarePath rewrites legacy /console/* paths to the default-theme
// equivalents when the active theme is "default". For other themes the
// path is returned unchanged. The function only touches known prefixes
// so it is safe to call with arbitrary suffixes and query strings.
